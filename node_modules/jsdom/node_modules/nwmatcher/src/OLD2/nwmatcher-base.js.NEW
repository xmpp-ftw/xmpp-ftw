      ...
      while (selector) {
        ...
        if ((match = selector.match(Patterns.universal))) {
          ...
          if (match[2] && match[4] && (type = Operators[match[2]])) {
            test = match[4].replace(/(\x22|\x27)/g, '\\$1');
            test = test.replace(/\\([0-9a-f]{2,2})/g, '\\x$1');
            expr = 'n=s.getAttribute(e,"' + name + '").toLowerCase();';
            type = type.replace(/\%m/g, test.toLowerCase());
          } else if (!match[2]) {
            expr = 'n=s.hasAttribute(e,"' + name + '");';
            type = 'n';
/*
            expr = 'n=e.' + (IE_LT_9 ?
              'getAttributeNode("' + name + '");' :
              'hasAttribute("' + name + '");');
            type = IE_LT_9 ? ATTR_DEFAULT[name] ?
              '!!e.' + ATTR_DEFAULT[name] :
              'n&&(n.specified||n.nodeValue)' : 'n';
*/
          } else if (match[2] == '!=' || match[2] == '=') {
            test = match[4].replace(/(\x22|\x27)/g, '\\$1');
            expr = 'n=e.getAttribute("' + name + '");';
            type = 'n' + match[2] + '="' + test + '"';
          }
          source = expr + 'if(' + type + '){' + source + '}';
//if (name == 'multiple') alert(source);
        }
