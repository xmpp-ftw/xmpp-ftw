...
PubSub.prototype._eventNotification = function(stanza) {
    ...
    if (event.hasChild('items')) return this._itemNotification(stanza, event)
    return false
}

PubSub.prototype._itemNotification = function(stanza, event) {
    var items = event.getChild('items')
    var data = {}
    this._getItemData(items, data)
    if (stanza.hasChild('headers')
       this._getHeaderData(stanza.getChild('headers'), data)
