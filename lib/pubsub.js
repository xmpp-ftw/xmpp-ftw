...
PubSub.prototype.handle = function(stanza) {
    return this._eventNotification(event)
}
...
PubSub.prototype._eventNotification = function(event) {
    if (event.hasChild('items')) return this._itemNotification(event)
    return false
}

PubSub.prototype._itemNotification = function(event) {
    var items = event.getChild('items')
    var data = {}
    this._getItemData(items, data)
    if (event.hasChild('headers')
       this._getHeaderData(event.getChild('headers'), data)
    this.socket.emit('xmpp.pubsub.push.item', data)
}

PubSub.prototype._getHeaderData = function(headers, data) {
    data.headers = []
...
PubSub.prototype._getItemData = function(items, data) {
    ...
    data.from = items.attrs.from
    if (!(item = items.getChild('item'))) {
        console.error('Invalid item notification')
        return false
    }
