...
Presence.prototype.handleError = function(stanza) {
    var description = ''
    if (stanza.getChild('text'))
        description = stanza.getChild('text').getText()
    var from = stanza.attrs.from
    if (0 == description.length)
        description = stanza.getChild('error').children[0].getName()
    this.socket.emit(
        'xmpp.presence.error',
...
Presence.prototype.subscription = function(jid, type) {
    if (!jid) return this._clientError("Missing 'to' key")
